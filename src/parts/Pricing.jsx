...
const Pricing = ({section,data}) => {
  ...
  {data.categories.map((item) => (
    <tr key={item.name} className='content-table'  id='table-border'>
        ...
        <td className='center-td' style={{ width: "25%"}}  id='table-border'>
            <h4 className='text-price'>Rp {item.price.web ? `${item.price.web.min}-${item.price.web.max}jt` : '-'}</h4>
        </td>
  ...
  {data.categories.map((item) => (
    <tr key={item.name} className='content-table'  id='table-border'>
        ...
        <td className='center-td' style={{ width: "25%"}}  id='table-border'>
            <h4 className='text-price'>Rp {item.price.android ? `${item.price.android.min}-${item.price.android.max}jt` : '-'}</h4>
        </td>
  ...
  return (
    <section id={section} className='my-32'>
      <div style={{ gap: 16 }}>
        ...
        <p className="sub-title-team"style={{ marginBottom:"32px" }}>{data.description}</p>
        <div className='d-lg-none d-md-block'>
        <Tabs tabsData={tabsData} display={display} handleActive={handleStatus} />
        </div>
    <div className='d-none d-lg-block'>

      <table className="table table-bordered" id='table-border'>
 
      <tbody>
    <tr className='text-center'  id='table-border'>
        <td rowspan="2" style={{width: "50%"}}></td>
        <td colspan="2" className='title-estimated' style={{width: "50%",letterSpacing:"-7%"}}  id='table-border'>Estimated price</td>     
    </tr>
    <tr className='text-center'  id='table-border'>
        <td className='title-estimated' id='table-border' style={{ letterSpacing:"-7%" }}>Web</td>
        <td className='title-estimated'  id='table-border' style={{ letterSpacing:"-7%" }}>Android App</td>
    </tr>
    {data.categories.map((item) => (
